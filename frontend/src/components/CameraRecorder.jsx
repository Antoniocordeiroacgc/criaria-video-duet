...
export default function CameraRecorder({ onRecordingComplete, referenceFile, referenceMode, onRecordingStart, carouselRef, referenceVideoRef, camFilterCss = 'none' }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full flex flex-col gap-4">
      <div className="relative w-full aspect-[9/16] max-h-[70vh] bg-black rounded-xl overflow-hidden border border-border/50 shadow-lg">
        {isStreamReady && stream ? (
          <video
            ...
            style={{ filter: camFilterCss, transition: 'filter 0.3s ease' }}
          />
        ) : (
        ...
        {isRecording && !isPaused && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute top-4 left-4 flex items-center gap-2 bg-destructive/90 text-white px-3 py-1.5 rounded-lg shadow-sm">
        ...
        {isRecording && !isPaused && referenceMode === 'video' && referenceFile && (
          <div className="absolute bottom-4 left-0 right-0 flex justify-center">
            <button onClick={handleShowReference} disabled={refPlaying} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold shadow-lg transition-all ${refPlaying ? 'bg-green-600/80 text-white cursor-default' : 'bg-white/90 text-black hover:bg-white'}`}>
              ...
              {refPlaying ? `Referência tocando (${refStartTimestamp?.toFixed(1)}s)` : 'Mostrar Referência'}
            </button>
          </div>
        )}
      </div>
      ...
      {error && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="p-3 bg-destructive/10 border border-destructive/20 rounded-lg">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            {!isStreamReady ? (
              <Button onClick={startCamera} size="lg" className="bg-primary hover:bg-primary/90 text-primary-foreground font-semibold shadow-md">
                <Camera className="w-5 h-5 mr-2" />Ligar Câmera
              </Button>
            ) : (
              <>
                <Button onClick={handleStopCamera} size="lg" variant="secondary" className="font-medium">
                  <CameraOff className="w-5 h-5 mr-2" />Desligar Câmera
                </Button>
                {!isRecording ? (
                  <Button onClick={handleStartRecording} size="lg" className="bg-destructive hover:bg-destructive/90 text-destructive-foreground font-semibold shadow-md">
                    <Circle className="w-5 h-5 mr-2 fill-current" />Gravar
                  </Button>
                ) : (
                  <>
                    <Button onClick={isPaused ? resumeRecording : pauseRecording} size="lg" variant="secondary" className="font-medium border-border">
                      {isPaused ? <><Play className="w-5 h-5 mr-2 fill-current" />Retomar</> : <><Pause className="w-5 h-5 mr-2 fill-current" />Pausar</>}
                    </Button>
                    <Button onClick={stopRecording} size="lg" className="bg-destructive hover:bg-destructive/90 text-destructive-foreground font-semibold shadow-md">
                      <Square className="w-5 h-5 mr-2 fill-current" />Parar Gravação
                    </Button>
                  </>
                )}
              </>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col gap-4 p-5 bg-card border border-border shadow-sm rounded-xl">
            <div className="flex items-center justify-between border-b border-border/50 pb-3">
              <h3 className="text-base font-semibold text-foreground flex items-center gap-2">
                <CheckCircle2 className="w-5 h-5 text-green-500" />Gravação concluída
              </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-1">
              <Button onClick={handleDownload} size="lg" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-semibold">
                <Download className="w-5 h-5 mr-2" />Baixar Vídeo Gravado
              </Button>
              <Button onClick={handleSendForComposition} size="lg" variant="secondary" className="w-full font-semibold border-border"
            ...
            {submitStatus === 'done' && downloadUrl && (
              <a href={downloadUrl} download className="w-full inline-flex items-center justify-center gap-2 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold rounded-lg py-3">
                <Download className="w-5 h-5" />Baixar Vídeo Duet Final
              </a>
            )}
